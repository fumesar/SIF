...
class CalificacionControlador extends Controller
{
    ...
    public function report(Request $request){
        ...
        if ($request){
            ...
            $calificaciones = Calificacion::where('idSemestre',$query_idSemestre)->where('idCurso',$query_idCurso)->get();
            
            $calificacions=DB::table('matricular')

                ->select('matricular.idMatricula as idMatricula','jefatura.idJefatura','matricular.Documento','matricular.Nombres as Nombres','matricular.Apellidos as Apellidos',DB::RAW('"" as calificaciones'),DB::RAW('0 as promedio'))
            ...
            foreach($calificacions as $calificacion){
                ...
                foreach($asignaturas as $asignatura){
                    $nota=$calificaciones
                        ...
                        ->where('idJefatura',$calificacion->idJefatura)
                        ->where('Nota','<>',null)
                        ->first();
                    if($nota == null )
                        if( $notas == '')
                    ...
                    $sumatoria = $sumatoria + ($nota ? $nota->Nota : 0) ;
                }
                if(substr($notas,-1) !== ',')
                    $notas.=',';
                $promedio = round($sumatoria / $asignaturas->count(),1);
                $calificacion->promedio=$promedio;
                //dd($sumatoria,$asignaturas->count(),$promedio);
                $calificacion->calificaciones = '['.$notas.$promedio.']';
                //if($calificacion->idJefatura == 57)
                //    dd($calificacion->calificaciones);
                //dd(json_decode($calificacion->calificaciones));
            }
            $calificacions=$calificacions->sortByDesc('promedio');
            //dd($calificacions);
